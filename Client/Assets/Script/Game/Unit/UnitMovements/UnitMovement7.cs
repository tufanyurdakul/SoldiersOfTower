...
public class UnitMovement7 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            case 1:
                rgbUnit.velocity = new Vector2(-1.3f, 0) * x2;
                enemyState = 1;
                break;
            case 0:
                rgbUnit.velocity = new Vector2(1.3f, 0) * x2;
    ...
    void Update()
    {
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("Unit7Attack"))
        {
            ...
            if (atackSpeedTimer <= 0)
            {
                if (enemyUnitFunctions != null && enemyStats != null)
                {
                    if (enemyState == 0)
                    {
                        int randomAttack = (stats.AttackDamage + stats.AbilityPower) + (int)((float)enemyStats.Health * (0.01f + ((float)stats.AbilityPower * 0.01f)));
                        int damage = enemyUnitFunctions.HitPhysicalDamage(randomAttack);
                        ...
                        int LifeSteal = 2 + ((int)((float)(stats.MaxHealth - stats.Health) * (0.01f + ((float)stats.AbilityPower * 0.005f))));
