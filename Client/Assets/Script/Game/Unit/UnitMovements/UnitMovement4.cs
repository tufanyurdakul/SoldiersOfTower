...
public class UnitMovement4 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        switch (gameObject.transform.rotation.y)
        {
            case 1:
                ...
                enemyState = 1;
                attackEffect = Resources.Load("Effects/AttackEffectEnemy4") as GameObject;
                break;
            case 0:
                rgbUnit.velocity = new Vector2(1, 0) * x2;
                enemyState = 0;
                attackEffect = Resources.Load("Effects/AttackEffect4") as GameObject;
                break;
        }
    ...
    void Update()
    {
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("Unit4Attack"))
        {
            ...
            if (atackSpeedTimer <= 0)
            {
                if (enemyUnitFunctions != null && enemyStats != null && enemyState == 0)
                {
                    if (enemyState == 0)
                    {
                        ...
                        enemyUnitFunctions.Poisoned(0,stats.AbilityPower,0.5f,2.5f,4);
